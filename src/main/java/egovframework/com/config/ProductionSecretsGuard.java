 ...
 *
 * <p>암복호화 키(EGOV_CRYPTO_KEY)가 공개된 기본 placeholder 그대로이거나 권장 길이 미만이면
 * 애플리케이션 기동을 즉시 중단(fail-fast)한다. 기존에는 경고 로그만 남기고 기동되어,
 * 운영 환경에서도 기본키로 동작할 위험이 있었다.</p>
 *
 * <p>본 포털은 <b>세션 기반 Spring Security</b>를 사용하며 JWT 토큰을 사용하지 않으므로
 * JWT 시크릿은 검증 대상이 아니다(원본 portal-site-jsp와 동일한 세션 인증 방식).</p>
 *
...
public class ProductionSecretsGuard {
    ...
    /** application.properties 에 박혀 있는 기본 placeholder 값 (운영에서 사용 금지) */
    ...
    public void verify() {
        ...
        log.info("[보안] 운영 시크릿 검증 통과 (Crypto 키가 기본값이 아님).");
    }
}
