...
public class EgovFileMngController {
	...
	/**
	 * 암호화되어 전달되는 atchFileId 파라미터를 바인딩 시 복호화한다.
	 * (JSP 원본은 egov-com-servlet.xml 의 ConfigurableWebBindingInitializer 로 전역 등록되어 있으나,
	 *  Boot 전환본은 평문 atchFileId 를 hidden 으로 전달하는 폼(FAQ 등)이 있어
	 *  파일관리 컨트롤러에만 국한하여 등록한다.)
	 */
	...
	public String selectFileInfs(@ModelAttribute("searchVO") FileVO fileVO,
		...
		if (param_atchFileId != null && !"".equals(param_atchFileId)) {
			byte[] encrypted_atchFileId = Base64.getDecoder().decode(param_atchFileId.replaceAll(" ", "+"));
	...
	public String selectFileInfsForUpdate(@ModelAttribute("searchVO") FileVO fileVO,
		...
		model.addAttribute("fileListCnt", result.size());
		model.addAttribute("atchFileId", param_atchFileId);
		// 삭제 후 복귀할 URL (호출 화면에서 전달, JSP 원본의 JS returnUrl 생성을 대체)
	...
	public String deleteFileInf(@ModelAttribute("searchVO") FileVO fileVO, @RequestParam("returnUrl") String returnUrl,
		...
		// returnUrl 미전달/외부 URL 방지 (open redirect 차단)
		if (StringUtils.isBlank(redirectUrl) || !redirectUrl.startsWith("/") || redirectUrl.startsWith("//")) {
			redirectUrl = "/";
		}
