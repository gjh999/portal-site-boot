...
public class EgovBBSManageController {
	...
	public String selectBoardArticles(@ModelAttribute("searchVO") BoardVO boardVO, ModelMap model,
		...
		//-------------------------------
		// 갤러리(BBSA02) 목록 썸네일: getImage.do 호출용으로 atchFileId 를 세션 암호화하여 전달
		//-------------------------------
		if ("BBSA02".equals(master.getBbsAttrbCode())) {
			@SuppressWarnings("unchecked")
			List<BoardVO> articleList = (List<BoardVO>) map.get("resultList");
			String sessionId = request.getSession().getId();
			for (BoardVO article : articleList) {
				if (article.getAtchFileId() != null && !"".equals(article.getAtchFileId())) {
					article.setAtchFileId(EgovFileMngController.encryptSession(article.getAtchFileId(), sessionId));
				}
			}
		}
		////-----------------------------
	...
	public String selectBoardArticle(@ModelAttribute("searchVO") BoardVO boardVO, ModelMap model,
		...
		//----------------------------
		// 첨부파일 목록 (JSP 원본의 <c:import url="/cmm/fms/selectFileInfs.do"> 대체)
		// cmm/fms/EgovFileList :: fileList 프래그먼트를 th:replace 로 포함하기 위해
		// 모델에 fileList(다운로드 링크용 - 항목별 atchFileId 는 세션 암호화)를 담는다.
		//----------------------------
		if (vo.getAtchFileId() != null && !"".equals(vo.getAtchFileId())) {
			String sessionId = request.getSession().getId();

			model.addAttribute("atchFileIdEnc", EgovFileMngController.encrypt(vo.getAtchFileId()));

			FileVO fileVO = new FileVO();
			fileVO.setAtchFileId(vo.getAtchFileId());
			List<FileVO> fileList = fileMngService.selectFileInfs(fileVO);
			for (FileVO file : fileList) {
				file.setAtchFileId(EgovFileMngController.encryptSession(vo.getAtchFileId(), sessionId));
			}
			model.addAttribute("fileList", fileList);

			//----------------------------
			// 갤러리(BBSA02)는 이미지 첨부 미리보기 목록 추가
			// (JSP 원본의 <c:import url="/cmm/fms/selectImageFileInfs.do"> 대체,
			//  cmm/fms/EgovImgFileList :: imgFileList 프래그먼트용. getImage.do 는 세션 암호화 ID 요구)
			//----------------------------
			if ("BBSA02".equals(vo.getBbsAttrbCode())) {
				model.addAttribute("atchFileIdImgEnc",
						EgovFileMngController.encryptSession(vo.getAtchFileId(), sessionId));

				FileVO imgVO = new FileVO();
				imgVO.setAtchFileId(vo.getAtchFileId());
				List<FileVO> imgFileList = fileMngService.selectImageFileList(imgVO);
				for (FileVO file : imgFileList) {
					file.setAtchFileId(EgovFileMngController.encryptSession(vo.getAtchFileId(), sessionId));
				}
	...
	public String selectBoardArticleForUpdt(@ModelAttribute("searchVO") BoardVO boardVO,
		...
		//----------------------------
		// 기존 첨부파일 목록 (JSP 원본의 <c:import url="/cmm/fms/selectFileInfsForUpdate.do"> 대체)
		// cmm/fms/EgovFileList :: fileList 프래그먼트(updateFlag='Y')를 th:replace 로 포함하기 위해
		// 모델에 fileList 와 삭제 폼 전송용 암호화 atchFileId(atchFileIdEnc)를 담는다.
		//----------------------------
		if (bdvo.getAtchFileId() != null && !"".equals(bdvo.getAtchFileId())) {
			model.addAttribute("atchFileIdEnc", EgovFileMngController.encrypt(bdvo.getAtchFileId()));
	...
	public String updateBoardArticle(final MultipartHttpServletRequest multiRequest,
		...
		if (isAuthenticated) {
			...
			// 새 파일을 업로드하지 않더라도 기존 첨부파일 ID를 유지 (DB 조회 값 사용)
			board.setAtchFileId(atchFileId);
